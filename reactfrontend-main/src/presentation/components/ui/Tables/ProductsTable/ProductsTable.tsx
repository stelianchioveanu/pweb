...
import { ProductAddDialog } from "../../Dialogs/ProductTagDialog";
...
const useHeader = (): { key: keyof ProductDTO, name: string }[] => {
    ...
    return [
        ...
        { key: "tags", name: formatMessage({ id: "globals.tags" }) },
    ]
};
...
export const ProductsTable = () => {
    ...
    const { handleChangePage, handleChangePageSize, pagedData, isError, isLoading, tryReload, labelDisplay, remove } = useProductsTableController(); // Use the controller hook.
    const rowValues = getRowValues(pagedData?.data, orderMap); // Get the row values.

    return <DataLoadingContainer isError={isError} isLoading={isLoading} tryReload={tryReload}> {/* Wrap the table into the loading container because data will be fetched from the backend and is not immediately available.*/}
        {/* <ProductAddDialog /> Add the button to open the user add modal. */}
                        ...
                        rowValues?.map(({ data, entry }, rowIndex) => <TableRow key={`row_${rowIndex + 1}`}>
                            {data.map((keyValue, index) => <TableCell key={`cell_${rowIndex + 1}_${index + 1}`}>
                                {Array.isArray(keyValue.value) ? (
                                    keyValue.value.map((item: any, idx: number) => 
                                        `${item} `
                                    )
                                ) : (
                                    keyValue.value
