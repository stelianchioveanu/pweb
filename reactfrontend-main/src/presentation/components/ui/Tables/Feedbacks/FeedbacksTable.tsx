...
import { FeedbackDTO, ProductDTO, ProductTagDTO } from "@infrastructure/apis/client";
...
const useHeader = (): { key: keyof FeedbackDTO, name: string }[] => {
    ...
    return [
        { key: "description", name: formatMessage({ id: "globals.id" }) },
        { key: "stars", name: formatMessage({ id: "globals.id" }) },
    ]
};
...
export const FeedbacksTable = () => {
    ...
    const { handleChangePage, handleChangePageSize, pagedData, isError, isLoading, tryReload, labelDisplay } = useFeedbacksTableController(); // Use the controller hook.
